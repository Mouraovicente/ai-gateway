...
- **Two layers of rate limiting**: per-tenant after auth, plus a pre-auth per-IP bucket charged only by auth *failures* (60/min default), so brute force never reaches the tenant store — [`internal/ratelimit`](internal/ratelimit). Behind a NAT/shared egress IP, many clients share that one bucket, so a single brute-forcer can 429 the rest until it refills; tune with `IP_AUTH_FAILURES_PER_MINUTE`.
